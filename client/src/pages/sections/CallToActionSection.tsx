
import React from "react";
import { Card, CardContent } from "@/components/ui/card";

export const CallToActionSection = (): JSX.Element => {
  // Testimonial data for mapping
  const testimonials = [
    {
      id: 1,
      name: "Francis George",
      role: "Customer",
      image: "/figmaAssets/frame-39.svg",
      text: "our valued customers share their honest experiences with Narpavai Honey—from taste to trust, and everything in between.",
      stars: "/figmaAssets/frame-531686.svg",
    },
    {
      id: 2,
      name: "JASMINE ALBERT",
      role: "Customer",
      image: "/figmaAssets/frame-39-1.svg",
      text: "Discover what our valued customers are saying about their experience with Narpavai Honey—pure, flavorful, and trusted by families nationwide.",
      stars: "/figmaAssets/frame-531686.svg",
    },
  ];

  return (
    <section className="relative w-full bg-[#fffae2] py-12">
      <div className="container relative mx-auto bg-[url(/figmaAssets/untitled-design--12--1-1.svg)] bg-cover bg-center">
        <div className="flex flex-col lg:flex-row gap-8 items-center">
          {/* Left content - Just heading */}
          <div className="lg:w-1/3 space-y-8">
            <h2 className="font-['Alata',Helvetica] font-normal text-[#7e4a1d] text-5xl leading-[57.6px]">
              What Our <br />
              Customer Say&apos;s
            </h2>
          </div>

          {/* Right content - Testimonial cards */}
          <div className="lg:w-2/3 flex flex-col md:flex-row gap-6">
            {testimonials.map((testimonial) => (
              <Card
                key={testimonial.id}
                className="w-full md:w-[542px] h-[528px] bg-cover bg-center rounded-[30px] border-none overflow-hidden relative"
                style={{
                  backgroundImage: `url(/figmaAssets/rectangle-3463901.png)`,
                }}
              >
                <CardContent className="p-0 h-full relative">
                  <div className="relative w-full h-full pt-[87px] px-[27px]">
                    <div className="flex flex-col items-center">
                      {/* Customer image */}
                      <div className="w-[156px] h-[156px] mx-auto rounded-full bg-cover bg-center mb-6" 
                           style={{ backgroundImage: `url(${testimonial.image})` }}>
                      </div>
                      
                      {/* Customer text */}
                      <p className="w-[302px] mx-auto text-[#131313] text-base text-center leading-[20.8px] font-['Poppins',Helvetica] mb-16">
                        {testimonial.text}
                      </p>
                      
                      {/* Star rating */}
                      <div className="mb-6">
                        <img className="w-[82px] h-4" alt="Five star rating" src={testimonial.stars} />
                      </div>
                      
                      {/* Customer name and role */}
                      <div className="text-center">
                        <p className="font-['Poppins',Helvetica] font-medium text-[#131313] text-lg leading-[30px]">
                          {testimonial.name}
                        </p>
                        <p className="font-['Poppins',Helvetica] text-[#131313] text-sm leading-[30px]">
                          {testimonial.role}
                        </p>
                      </div>
                    </div>
                  </div>
                </CardContent>
              </Card>
            ))}
          </div>
        </div>
      </div>
    </section>
  );
};
